...
describe("<Grid />", () => {
  ...
  it("should render next generation button and reset button", () => {
    render(<Grid />);
    expect(
      screen.getByRole("button", { name: /next generation/i })
    ).toBeVisible();
  ...
  it("should generate properly when 'next generation' button is clicked", () => {
    ...
    user.click(cell0404);
    const generationButton = screen.getByRole("button", {
      name: /next generation/i,
    });
    ...
    //die because of less neighbors
    expect(cell0404).toHaveStyle(`background: lightblue;`);
    //come to life because of getting exactly 3 neighbors
    expect(screen.getByRole("button", { name: /cell-3-5/i })).toHaveStyle(
      `background: darkblue;`
    );
  });
  ...
  it("should generate properly when 'next generation' button is clicked when a few border cells are live", () => {
    ...
    //come to life because of getting exactly 3 neighbors
    expect(screen.getByRole("button", { name: /cell-11-0/i })).toHaveStyle(
      `background: darkblue;`
    );
    expect(screen.getByRole("button", { name: /cell-11-11/i })).toHaveStyle(
      `background: darkblue;`
    );
  });

  //TODO: add more edge cases for testing next generation with wrapped border neighbor cells
});
